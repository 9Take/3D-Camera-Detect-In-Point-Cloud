...
โปรเจกต์นี้มี [`CLAUDE.md`](../../CLAUDE.md) ที่ระบุกติกาในการทำงาน สรุปสั้น ๆ:
- **เปลี่ยนแบบผ่าตัด** — แตะเฉพาะที่งานต้องการ ทำตามสไตล์เดิม
- **เรียบง่ายไว้ก่อน** — เขียนโค้ดน้อยที่สุดที่แก้ปัญหาได้
- **ตรวจสอบด้วยเป้าหมาย** — เช่น "รัน trigger หนึ่งครั้ง แล้วเช็กค่า slot ใน `position_mem.json`"

สิ่งแรก ๆ ที่ควรลอง:
1. `python main.py --debug` กด `1` แล้ว `t` ดูการสแกนทำงานครบตั้งแต่ต้นจนจบ (ถ้าไม่มี PLC
   การเขียนค่าไป PLC จะ fail เงียบ ๆ ซึ่งไม่เป็นไรสำหรับการดูเส้นทางฝั่ง vision)
2. เปิด `data/logs/position_mem.json` หลังสแกน เพื่อดูผลลัพธ์ที่ส่งออกไปจริง
3. อ่าน [main-loop.th.md](main-loop.th.md) ควบคู่กับเปิด `main.py` ไปด้วย
