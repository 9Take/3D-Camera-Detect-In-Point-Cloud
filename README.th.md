...
> README นี้อธิบายภาพรวมการทำงานของทั้งระบบ แยกตามส่วนงาน เพื่อให้
...
PLC สั่งให้ระบบ "scan now โดยใช้โมเดล/โปรแกรมหมายเลข N" จากนั้นเราดึงภาพ RGB-D
...
1. **เริ่มจากการไหลของข้อมูล (data flow) ไม่ใช่ไล่อ่านทีละไฟล์** ให้เปิด
   [main-loop.th.md](docs/handover/main-loop.th.md) ก่อน เอกสารนี้อธิบายการทำงานครบหนึ่งรอบ
   ตั้งแต่ PLC ส่งบิตสั่งงาน (trigger) ไปจนถึงตอนเขียนผลลัพธ์กลับให้ PLC เมื่อเข้าใจรอบนี้แล้ว
   ส่วนที่เหลือจะเข้าใจง่ายขึ้น เพราะทุกโมดูลต่อยอดมาจากรอบการทำงานนี้
2. **รันในโหมด `--debug` โดยไม่ต้องมี PLC** สั่ง trigger จากคีย์บอร์ดได้
   และดูหน้าต่าง live + 3D viewer ได้ ดู [getting-started.th.md](docs/handover/getting-started.th.md)
3. **ฝั่ง PLC รีจิสเตอร์** ข้อตกลงทั้งหมดกับ PLC อยู่ในส่วน
